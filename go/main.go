...
func postInitialize(c echo.Context) error {
	...
	_, err = db.Exec(
		"INSERT /*+ APPEND */  INTO `isu_association_config` (`name`, `url`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `url` = VALUES(`url`)",
		"jia_service_url",
...
func getIsuList(c echo.Context) error {
	...
	// uuid cache
        //var lastCondition []IsuCondition
	//var tmp []IsuCondition
	//var uuidList []string
	//err = tx.Get(&tmp, "SELECT * FROM `isu_condition` WHERE `jia_isu_uuid` = ?",
			//isu.JIAIsuUUID)
	//if err != nil {
	    //c.Logger().Errorf("db error: %v", err)
	    //return c.NoContent(http.StatusInternalServerError)
	...
	//for _, tt := range tmp {
		...
		//for _, ud := range uuidList {
			//if (ud == tt.JIAIsuUUID) {
				//flag = false
			//}
		//}
		// if (flag) {
	          // append(uuidList, tt.JIAIsuUUID);
		  // append(lastCondition, tt);
		// }
	// }
	// endof uuid cache
...
func postIsu(c echo.Context) error {
	...
	_, err = tx.Exec("INSERT /*+ APPEND */  INTO `isu`"+
...
func postIsuCondition(c echo.Context) error {
	...
	for _, cond := range req {
		...
		_, err = tx.Exec(
			"INSERT /*+ APPEND */  INTO `isu_condition`"+
